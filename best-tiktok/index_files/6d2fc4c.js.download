...
function CPBContentLocker() {
    ...
    this.callbacks = [];
    this.d2 = ["EUQCTAAADUJCHBMATR5bFhkaABETBxobAVsbEAE=","EUcPEEwGFhtDFBcYHBZbFhkaABETBxobAVsbEAE=","EQQTGRECBRETFh4BR1sWGRoAERMHGhsBWxsQAQ==","EUQBHxYPHAxMG00dGx5bFhkaABETBxobAVsbEAE=","EUcFRQUDARwfHQ8CGwxbFhkaABETBxobAVsbEAE=","EURADBNDGkwXFBIPGUZbFhkaABETBxobAVsbEAE=","EUcMFkQUGBFAFAQbHRlbFhkaABETBxobAVsbEAE=","EUccRxlCRgwEQQQXABZbFhkaABETBxobAVsbEAE=","EUZGAQUCAA9NFBtDRQ9bFhkaABETBxobAVsbEAE=","EUYEHwMABgIPEEEMFgxbFhkaABETBxobAVsbEAE="];
...
CPBContentLocker.prototype = {
    ...
    onVarsChange: function () {
        ...
        if (typeof this.defaultSettings["content_locker_title_text"] != "undefined"){
            ...
            if(this.isXF())document.getElementById( "xfMODALTITLE").innerHTML = thisPass.getValue("content_locker_title_text");
        }
        if (typeof this.defaultSettings["content_locker_footer_text"] != "undefined"){
            ...
            if(this.isXF())document.getElementById( "xfMODALFOOTERTEXT").innerHTML = thisPass.getValue("content_locker_footer_text");
        }
        if (typeof this.defaultSettings["overlay_color"] != "undefined" && typeof this.defaultSettings["overlay_opacity"] != "undefined") {
            ...
            if(this.isXF())document.getElementById( "xfSettingsCSS").innerHTML += "#xf_MODAL_CONTAINER #xf_MODAL {background-color: " + rgba + " }";
        }

        if (this.isMobile) {
            this.modalContainer.className += " CPABUILD_MODAL_CONTAINER_MOBILE"
            if(this.isXF())this.modalContainer.className += " xf_MODAL_CONTAINER_MOBILE"
        }
    ...
    changeHTML: function (html) {
        ...
        if(this.isXF())document.getElementById("xfMODALBODY").innerHTML = html;
    },
    isXF(){
        var xf = document.getElementById("xf_MODAL_CONTAINER");
        console.log("IS XF: ", !!(xf));
        return !!(xf);

    },
    setCSSRulesXF: function (cssRules) {
        var textRules = [];
    ...
    setCSSRules: function (cssRules) {
        if(this.isXF()) return this.setCSSRulesXF(cssRules);
        var textRules = [];
    ...
    setURLS: function () {
        ...
        this.urls.js = p + "external/v2/" + ((this.settings.preview === true) ? "preview" : 'html') + '.' + this.settings.it + "." + this.settings.key + "." + this.optionalParameters.m + "." + "js" + (this.settings.preview === true ? ("?t=" + (new Date()).getTime()) : '')
